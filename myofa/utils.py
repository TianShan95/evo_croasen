...
class MyNetwork(MyModule):
    ...
    def gcn_forward(self, x, adj, gcn_blocks, out_gcn_vector='sum', embedding_mask=None, device='cpu'):
    # 把gcnblocks传入进来 进行图卷积
        ''' Perform forward prop with graph convolution.
        Returns:
            Embedding matrix with dimension [batch_size x num_nodes x embedding]
        '''

        # type(self.dropout_rate)
        x_all = []
        for gcn_net in gcn_blocks:
            ...
            # # 构建一个 gcn 模型
            # gcn_graph = hl.build_graph(gcn_net, (x, adj))
        ...
        #     x_tensor = x
        #
        x_tensor = torch.tensor(torch.zeros(x.size(0), x.size(1), x.size(2))).to(device)
        if out_gcn_vector == 'sum' or out_gcn_vector == 'mean':
            for x in x_all:
                x_tensor += x
            if out_gcn_vector == 'mean':
                x_tensor = x_tensor / 3

        elif out_gcn_vector == 'max':
            x_tensor = x_all[0]
            for i, x in enumerate(x_all[:-1]):
                x_tensor = torch.where(x_tensor > x_all[i+1], x_tensor, x_all[i+1])
