...
# can报文卷积
from can_conv.structure_net import StrNN
import can_conv.data as data
...
def main(prog_args):
    ...
    with open('../../experiment/exp-record.txt', 'a') as f:
        ...
        f.close()
    # 定义此次实验的 log 文件夹
    log_out_dir = prog_args.out_dir + '/' + 'Rl_' + time_mark + '_with_conv_log/'
    ...
    can_conv_net = StrNN(gene_list).to(device)
    can_conv_net.load_state_dict(torch.load(can_conv_net_dir))
    conv_frame_num = 29
    ...
    if prog_args.mode == 'test':
        ...
        while True:
            ...
            # next_state, reward, train_done, val_done, label, pred, graph_loss = graph_task.benchmark_task_val(prog_args.feat, len_can)
            # 累加 奖励
            # ep_r += reward
            # 更新 状态
            # state = next_state
            ...
            for singleCan in range(prog_args.graph_batchsize):
                ...
                len_can = np.random.choice(prog_args.msg_biggest_num - prog_args.msg_smallest_num + 1,
                                           1, action.tolist())[0] + prog_args.msg_smallest_num
            ...
            next_state, reward, train_done, val_done, label, pred, graph_loss, graph_ypred, sample_graphs_list, last_canlen = \
                graph_task.benchmark_task_val(prog_args.feat, len_can_list)
            state = next_state
            print(graph_ypred)

            if val_done:
                print(f'pred_same_correct: {pred_same_correct}')
                print(f'pred_graph_correct: {pred_graph_correct}')
                print(f'pred_conv_correct: {pred_conv_correct}')
                print(f'total correct: {pred_same_correct+pred_graph_correct+pred_conv_correct}')

                print(f'pred_same_error: {pred_same_error}')
                print(f'pred_graph_error: {pred_graph_error}')
                print(f'pred_conv_error: {pred_conv_error}')
            ...
            for sample_num, sample_graph_len in enumerate(sample_num_per_lencan):
                ...
                if torch.argmax(graph_ypred[sample_num]) == torch.argmax(output_pre_sum_can_conv):
                    final_pred = torch.argmax(graph_ypred[sample_num])
                    count_same += 1
                    mark = 'same'
                else:
                    graph_gap = abs(graph_ypred[sample_num][0]-graph_ypred[sample_num][1])
                    conv_gap = abs((output_pre_sum_can_conv[0]-output_pre_sum_can_conv[1])/sample_graph_len)
                    print(f'graph_gap: {graph_gap} conv_gap: {conv_gap}')
                    if graph_gap >= conv_gap:
                        final_pred = torch.argmax(graph_ypred[sample_num])
                        count_graph += 1
                        mark = 'graph'
                    else:
                        final_pred = torch.argmax(output_pre_sum_can_conv)
                        count_conv += 1
                        mark = 'conv'

                # 判断是否判定正确
                if final_pred == label[sample_num]:
                    if mark == 'same':
                        pred_same_correct += 1
                        print(f'same  correct label: {label[sample_num]} pred: {final_pred}')
                    elif mark == 'graph':
                        pred_graph_correct += 1
                        print(f'graph correct label: {label[sample_num]} pred: {final_pred}')
                    elif mark == 'conv':
                        pred_conv_correct += 1
                        print(f'conv  correct label: {label[sample_num]} pred: {final_pred}')
                else:
                    if mark == 'same':
                        pred_same_error += 1
                        print(f'same   error  label: {label[sample_num]} pred: {final_pred}')
                    elif mark == 'graph':
                        pred_graph_error += 1
                        print(f'graph  error  label: {label[sample_num]} pred: {final_pred}')
                    elif mark == 'conv':
                        pred_conv_error += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_load_dir',
                        default='../../experiment/Rl_20220507_142946_multiDim_log/epoch_3_0.9979_20220507_150857', type=str)
