...
#  2021-12-09
#  1. 基因 0 - 7 可以接受全部的基因类型（较上一版 基因 5 7 包含了 sw5 和 dw 5）
#  2. kernel size 是 5 时，在基因 5 7 采用 stride 为 5 和 same padding
#  3. 完善 深度可分离 dw 卷积 在 深度卷积后，增加点乘卷积
#  备注：本版本无 5*5

operation_list = ['sw3', 'sw5', 'dw3', 'dw5', '3*3']
...
class StrNN(nn.Module):
    def __init__(self, individual_list):
        ...
        # 根据传进来的 list 构建 基因部分的 component
        for index, value in enumerate(self.individual_list):
            ...
            # 1. 根据 (value) 该位置 基因 的 种类 确定 padding
            ...
            # 2. 根据 (index) 是什么位置的 基因 来确定 通道
            if index <= 2:  # resA 不需要更改 padding 与 stride
                channel = 32  # resA 输入和输出通道 都是 32
            elif index <= 3:  # redA 不需要更改 padding 与 stride
                channel = 96  # redA 输入和输出通道 都是 96
            elif index <= 4:  # resB 不需要更改 padding 与 stride
                channel = 64  # resB 输入和输出通道 都是 64
            elif index <= 7:  # redB 输入和输出通道 都是 128
                channel = 128  # redB 需要特殊处理
                if index == 5 or index == 7:  # 特殊处理 redB 基因5 和 基因7 的padding 是 0
                    if size == 3:
                        padding = 0  # padding 特殊处理
                        stride = 3  # stride 特殊处理
                    if size == 5:
                        stride = 5  # stride 特殊处理 padding 不需要
    ...
    def forward(self, x):  # 输入 x： 29 * 29 * 1
        ...
        # print('x_resA_conv')
        # print(x_resA_conv1_1.shape)
        # print(x_resA_conv1_2.shape)
        # print(x_resA_conv1_3.shape)
        ...
        # print('2021-12-09 check')
        # print(x_redB_MaxPool_return.shape)
        # print(x_redB_conv3_return.shape)
        # print(x_redB_gen5_return.shape)
        # print(x_redB_gen7_return.shape)
        # print('end')
